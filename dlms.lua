...
local StructureValue = ProtoField.bytes("dlms.StructureValue","Elements",base.DOT)			-- Himanshu
...
function data_dissector(buffer, t_dataTree, offset, dataLen)
	...
	-- processing bit string						-- Himanshu
	if dataTypeIndex == 4  then
		local len = 4
		t_data:add(StringValue,buffer(offset+2,len))									-- dataLen
		offset = offset + len
	end
	...
	if dataTypeIndex == 9  then
		...
		local len = buffer(offset+2,1):uint()
		t_data:add(StringValue,buffer(offset+3,len))									-- dataLen
		offset = offset + 1 + len
	end
	...
	if dataTypeIndex == 1 or dataTypeIndex == 2 then
		...
		local t_Structure = t_data:add(StructureValue,buffer(offset+2,1))
		local n = buffer(offset+2,1):uint()
		-- t_Structure:add(StringValue,buffer(offset+3,dataLen-1))
		for i = 0,n-1,1 do
			offset = data_dissector(buffer, t_Structure, offset+2, dataLen-1)
		end
		return offset
	end
	return offset - 1
	...
	if dlms_type == 0xc4 then
		...
		if responseType == 1 then -- GetNormalResponse
			...
			if buffer(offset,1):uint() == 0 then -- CHOICE Data
				...
				data_dissector(buffer, t_data, offset, dataLen)			-- Himanshu
